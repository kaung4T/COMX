 ...
 * @return array
 */
function pleased_body_classes( $classes ) {
	...
	// Add a class for menu
	$menu_type = 'classic-menu' ;
	$classes[] = esc_attr( $menu_type );
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'pleased-selected-sidebar', true );
...
add_filter( 'body_class', 'pleased_body_classes' );
