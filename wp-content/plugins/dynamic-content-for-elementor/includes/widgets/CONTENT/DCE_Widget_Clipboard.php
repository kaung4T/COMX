...
class DCE_Widget_Clipboard extends DCE_Widget_Prototype {
    ...
    protected function render() {
        ...
        $this->add_render_attribute('input', 'id', 'dce-clipboard-value-' . $this->get_id());
        $this->add_render_attribute('input', 'class', 'elementor-field-textual');        

        $this->add_render_attribute('button', 'type', 'button');
        $this->add_render_attribute('button', 'id', 'dce-clipboard-btn-' . $this->get_id());
        $this->add_render_attribute('button', 'data-clipboard-target', '#dce-clipboard-value-' . $this->get_id());
            ...
            <?php if ($settings['dce_clipboard_type'] == 'code' && $settings['dce_clipboard_visible']) { ?>
             jQuery(document).ready(function () {
                if (wp.codeEditor) {
                    ...
                    var editor_<?php echo $this->get_id(); ?> = wp.codeEditor.initialize( jQuery('#dce-clipboard-value-<?php echo $this->get_id(); ?>'), editorSettings );
                }
            });
            <?php } ?>
            
            jQuery(document).ready(function () {
                var clipboard_<?php echo $this->get_id(); ?> = new ClipboardJS('#dce-clipboard-btn-<?php echo $this->get_id(); ?>');
                clipboard_<?php echo $this->get_id(); ?>.on('success', function (e) {
                    console.log(e);
                    jQuery('#dce-clipboard-btn-<?php echo $this->get_id(); ?>').addClass('animated').addClass('tada');
                    setTimeout(function(){
                        jQuery('#dce-clipboard-btn-<?php echo $this->get_id(); ?>').removeClass('animated').removeClass('tada');
                    }, 3000);
                });
                clipboard_<?php echo $this->get_id(); ?>.on('error', function (e) {
                    console.log(e);
                });
            });
        </script>
